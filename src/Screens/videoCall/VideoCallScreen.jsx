...
//     console.log("Setting up updateCallDuration listener");
//     const handler = (data) => {
      
//       setCallDuration(data.callDuration);
//       console.log('Got updateCallDuration in VideoCallScreen:', data.callDuration);
//     };
  
//     webSocket.on('updateCallDuration', handler);
  
//     return () => {
//       console.log("Cleaning up updateCallDuration listener");
//       webSocket.off('updateCallDuration', handler);
//     };
//   }, [webSocket]);
  

//   useEffect(() => {
//     console.log('callDuration updated:', callDuration);
//   }, [callDuration]);
...
const VideoCallScreen = ({ route, navigation }) => {
  ...
  // Listen for callDuration updates
  useEffect(() => {
    if (!webSocket) return;
    ...
    const handler = (data) => {
      console.log('Received updateCallDuration:', data);
      if (data && data.callDuration) {
        setCallDuration(data.callDuration);
        console.log('callDuration updated to:', data.callDuration);
      } else {
        console.warn('updateCallDuration event missing callDuration');
      }
    };
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (!config?.channelName || !config?.token || typeof config?.uid !== 'number') {
        console.error('Invalid config parameters');
        return;
      }
  ...
  const _renderVideos = () => (
    <View style={styles.fullView}>
      <View style={styles.splitContainer}>
        <View style={styles.remoteContainer}>{_renderRemoteVideos()}</View>
        {isCameraOn ? (
          <View style={styles.localContainer}>
            <RtcSurfaceView style={styles.local} canvas={{ uid: 0 }} />
          </View>
...
const styles = StyleSheet.create({
  max: { flex: 1, backgroundColor: 'black' },
  fullView: { flex: 1 },
