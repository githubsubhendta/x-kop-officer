...
//       console.log('VideoCall: updateCallDuration received', data.callDuration);
...
import ChatModal from '../../Components/chat/ChatModal.jsx';
import Svg, { Path } from 'react-native-svg';
import { useCall } from '../../context/callContext.js'; // Import useCall
...
const VideoCallScreen = ({ route, navigation }) => {
  const { config, mobile, chatId, userInfo } = route.params || {};
  const { callDuration, endCall: contextEndCall } = useCall(); // Use callDuration and endCall from context
  const [modelChat, setModelChat] = useState(false);
  const { engine: contextEngine } = useCall();
  const [isJoined, setJoined] = useState(false);
  const [peerIds, setPeerIds] = useState([]);
  const [connectionStatus, setConnectionStatus] = useState('Not Connected');
  const [isMicOn, setMicOn] = useState(true);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const initializeEngine = async () => {
        if (!_engine.current && isMounted) {
          await init();
        } else if (isMounted && _engine.current) {
          _engine.current.enableAudio();
          _engine.current.setEnableSpeakerphone(isSpeakerOn);
          _engine.current.muteLocalAudioStream(!isMicOn);
        }
  ...
  const endCall = async () => {
    try {
      ...
      navigate("EndCallScreen", { chatId: chatId, userInfo: userInfo });
    } catch (error) {
      console.error('Error ending call:', error);
      showMessage('Error ending call: ' + error);
    }
  ...
  return (
    <View style={styles.max}>
      ...
      <View style={styles.buttonHolder}>
        ...
        <View>
          <TouchableOpacity style={styles.button} onPress={() => setModelChat(true)}>
            <Svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 576 512"
              width={28}
